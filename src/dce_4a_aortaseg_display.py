...
def overlay(site, batch_no=None):
    base = [os.getcwd(), 'build', 'dce_4_aortaseg', site, "Patients"]
    if batch_no is not None:
        base.append(f"Batch_{batch_no}")
    dir = os.path.join(*base)

    map_base = [os.getcwd(), 'build', 'dce_2_data', site, "Patients"]
    if batch_no is not None:
        map_base.append(f"Batch_{batch_no}")
    map_dir = os.path.join(*map_base)


    mask_png_dir = os.path.join(os.getcwd(), 'build', 'dce_4_aortaseg', site, "Patients", 'Displays')
    ...
    aorta_database = db.series(dir, desc='DCE_4_aortaseg')

    map_database = db.series(map_dir, desc=f'DCE_1_kidneys')
    aorta = None
    for map_path in tqdm(map_database, desc=f'Creating {site} Aorta Seg Mosaics', unit='case'):
        case_id = map_path[1]
        tqdm.write(f'Processing case {case_id}...')

        try:
            aorta_mask = next(db.volume(m) for m in aorta_database if m[1] in map_path)
        except Exception as e:
            logging.error(f'cannot display aorta for case {case_id}: {e}')
        

        try:
            _map = db.volume(map_path, 'TriggerTime') 
